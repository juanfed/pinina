...
const T_mascotas = ({ t_mascotas, setT_mascotas, consultData, setConsultData }) => {
...
useEffect(() => {
    if (data.length > 0) {
        ...
        let filter = data.filter( item => {
            return item[ Object.keys(item)[0] ].toString().toLowerCase().includes(searchWord) || item.nombre_mascota.toString().toLowerCase().includes(searchWord) 
        });
        ...
        <DialogContent>
            <Grid container display="flex" justifyContent="center" alignItems="center" className="mt-5">
                <Grid item md={ 6 } xs={ 12 } sm={ 12 } className="mb-3">
                    <Typography variant="h5" className="text-center text-info">Mascotas</Typography>
                </Grid>
                ...
                { filterData.length > 0 && (
                    <>
                        <ListItem button key={ Object.keys(data[0])[0] }>
                            <ListItemText className="text-center text-info" style={{ width: '100%' }} primary={ 'nombre_mascota'.toUpperCase() } />
                            {/* <ListItemText className="text-center text-info" style={{ width: '100%' }} primary="Acciones"/> */}
                        </ListItem>
                        { filterData.map( item => (
                            <>
                                <Divider />
                                <ListItem button key={ Object.values(item)[0] } onClick={ () => handleClick(item.id_mascotas, item.nombre_mascota) } style={{ backgroundColor: (consultData.id_mascotas
                //REPLACECONSTRAINT
                 === item.id_mascotas ? '#e1e1e1' : null) }}>
                                    <ListItemText className="text-center" style={{ width: '100%' }} primary={ item.nombre_mascota } />
