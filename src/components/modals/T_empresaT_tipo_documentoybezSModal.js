...
const T_tipo_documento = ({ t_tipo_documento, setT_tipo_documento, consultData, setConsultData }) => {
...
const [ modalT_tipo_documento, setModalT_tipo_documento ] = useState(false);
...
useEffect(() => {
    ...
    if (t_tipo_documento && reload) {
        query();
        setReload(false);
    }
}, [ t_tipo_documento, reload ]);

useEffect(() => {
    if (data.length > 0) {

        let filter = data.filter( item => {
            return item[ Object.keys(item)[0] ].toString().toLowerCase().includes(searchWord) || item.nombre.toString().toLowerCase().includes(searchWord) 
        });
            ...
            setFilterData(data);
                ...
                const handleClick = (id, descripcion) => {
                    setConsultData({
                        ...
                        id_tipo_documentoDescripcion: descripcion
                    });
                }
                ...
                { filterData.length > 0 && (
                    <>
                        <ListItem button key={ Object.keys(data[0])[0] }>
                            <ListItemText className="text-center text-info" style={{ width: '100%' }} primary={ 'nombre'.toUpperCase() } />
                            {/* <ListItemText className="text-center text-info" style={{ width: '100%' }} primary="Acciones"/> */}
                        </ListItem>
                        { filterData.map( item => (
                            <>
                                <Divider />
                                <ListItem button key={ Object.values(item)[0] } onClick={ () => handleClick(item.id, item.nombre) } style={{ backgroundColor: (consultData.id_tipo_documento
                //REPLACECONSTRAINT
                 === item.id ? '#e1e1e1' : null) }}>
                                    <ListItemText className="text-center" style={{ width: '100%' }} primary={ item.nombre } />
