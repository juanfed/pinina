...
export default function ListaUbicaciones({ consultData, setConsultData }) {
    ...
    useEffect(() => {
        if (description.length !== 0) {
            ...
            if (filterData.length !== 0) {
                setConsultData({
                    ...consultData,
                    codigo_ubicacion_geografica_pais: filterData[0].id_codigo,
                    codigo_ubicacion_geografica_paisDescripcion: description
                });
            }
        }
    }, [description]);
        ...
        if (value.length !== 0) {
            ...
            if (filterData.length !== 0) {
                setConsultData({
                    ...consultData,
                    codigo_ubicacion_geografica_pais: filterData[0].id_codigo,
                    codigo_ubicacion_geografica_paisDescripcion: filterData[0].descripcion
                });
    ...
    useEffect(() => {
        if (checkValue) {
            if (consultData?.codigo_ubicacion_geografica_pais?.length !== 0) {
                setValue(consultData.codigo_ubicacion_geografica_pais)
                setCheckValue(false);
            }
        }
