...
export default function ListaUbicaciones({ consultData, setConsultData }) {
        ...
        if (value.length === 0) {
            if (description.length !== 0) {
                ...
                if (filterData.length !== 0) {
                    setConsultData({
                        ...consultData,
                        codigo_ubicacion_geografica: filterData[0].id_codigo,
                        codigo_ubicacion_geograficaDescripcion: description
                    });
                }
            }
        }
        ...
        if (value.length !== 0) {
            ...
            if (filterData.length !== 0) {
                setConsultData({
                    ...consultData,
                    codigo_ubicacion_geografica: filterData[0].id_codigo,
                    codigo_ubicacion_geograficaDescripcion: filterData[0].descripcion
                });
    ...
    useEffect(() => {
        if (checkValue) {
            if (consultData?.codigo_ubicacion_geografica?.length !== 0) {
                setValue(consultData.codigo_ubicacion_geografica)
                setCheckValue(false);
            }
        }
