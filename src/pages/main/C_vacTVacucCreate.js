...
import Historias from "../../components/MainPrincipal/Historias";
import VacunasDial from "../../layouts/speedDials/VacunasDial.jsx";
  ...
  const deleteElement = async () => {
    try {
      ...
      const { data } = await axiosClient.delete(
        "/t_vacunas/delete/" + elementToDelete
      );
      if (data.code === 0) {
        query();
  ...
  useEffect(() => {
    if (searchData.length !== 0) {
      const filterData = initialData.filter((i) => {
        return (
          i.id_tipo_vacunas.toLowerCase().includes(searchData) ||
