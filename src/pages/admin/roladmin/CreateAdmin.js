...
// Components
import OptionAdmin from './modals/OptionAdmin';
// Actions
import { AddAdminOpenAction, SearchUserBusinnessAction } from "../../../redux/actions/MainAction";
// Material-UI
...
const CreateAdmin = (props) => {
    ...
    const { addAdmin, adminUsuariosEmpresas } = useSelector( state => state.main );

    useEffect(() => {
        dispatch(SearchUserBusinnessAction());
        const query = async() => {
            try {
                const response = await axiosClient.post('/admin/userBusiness', { id: props.dataBusiness.id }); 
        ...
        <>
            <Typography variant="h6" className="text-center mt-3 mb-4">
                ...
                { addAdmin ?
                    ...
                    <IconButton className="mr-2" onClick={ () => dispatch(AddAdminOpenAction()) }>
                        <AddIcon style={{ fontSize: 30 }}/>
                    </IconButton>
                }
            </Typography>
            <Grid container justify="center" alignItems="center">
                <Grid item md={ 12 } sm={ 12 } xs={ 12 } className="text-center mb-3">
                    <TableContainer>
                        <Table stickyHeader aria-label="sticky table">
                            ...
                            <TableBody>
                                {   data.length > 0 && (
                                <>
                                    {   adminUsuariosEmpresas.map(item => (        
                                    <>
                                        {   data.map(item2 => (
                                        <>
                    ...
                    <TablePagination
                        ...
                        component="div"
                        count={adminUsuariosEmpresas.length}
